...
::: {.callout-note}
You can try out the binding from outside the container too: 

```{.bash}
apptainer exec -B ../data:/rawdata ../containers/fastqc:0.12.1.sif ls /rawdata
```

Will list the contents of `../data`, even though it's being called on as `rawdata`. 
:::
