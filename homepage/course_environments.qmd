...
Differing software versions and dependencies break reproducibility. We solve this for command-line bioinformatics tools with Pixi environments. (Module 1 showed the same problem, and the container solution, in R.)
...
To install Pixi for your user account, log in to the course server and execute this in the terminal:
...
::: {.callout-tip}
To source your shell, you need to source the startup files, in Linux it's the `source ~/.bashrc` file, in Mac it's the `source ~/.zshrc` file.
:::
...
```
Pixi will create a folder named `name_pixi_training` with a file `pixi.toml`. Let's have a look at that file!
...
```{.bash}
cd name_pixi_training
ls
less pixi.toml
```
To exit the `less` view, press `q` for quit.

Or use the file explorer of VScode. (o;
:::
...
If you are adding a platform that doesn't natively run on your OS (e.g. adding Linux when running on Windows) be sure to keep/add the OS you are running your system on!
:::
...
## pixi shell (does not work on hpc2n)
