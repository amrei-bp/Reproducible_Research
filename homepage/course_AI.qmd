...
In the past years, AI has become more mainstream. The primary kind of AI people think of, is a Large Language Model (LLM). At this point, you cannot really avoid contact with these models anymore as they have infiltrated every aspect of the internet.
...
![Token prediction of LLM's [source](https://albanna-tutorials.com/llm_agents.html)](figures/ai/token_prediction.jpg)

This is not an AI theory course, and we do not have the time to discuss the underlying theory beyond a rudimentary level. We would, however, like to discuss the impact of AI on you as a user, and advocate for responsible use of AI in your current and future work.
...
![GenAI chatbots by market share in September 2026 from [FirstPageSage](https://firstpagesage.com/reports/top-generative-ai-chatbots/)](figures/ai/Top-Generative-AI-Chatbots-by-MarketShare2026.webp)
:::
...
Most of the privacy and terms-of-service problems above are driven by one thing: when you use a commercial model your prompt — and any data attached to it — leaves your computer and lands on someone else's server. **Local models** avoid this. These are open-weight LLMs (for example Llama, Mistral, Qwen, Gemma, or DeepSeek) that you download and run on your own machine with a tool like [Ollama](https://ollama.com/), [LM Studio](https://lmstudio.ai/), or [llama.cpp](https://github.com/ggml-org/llama.cpp). This means that nothing you type leaves the machine.

This is great, because: 

- your prompts and data never travel to a third party — the single best mitigation for the sensitive-data concern above. You can describe a sequence or a table without it becoming training data or landing in a chat history you do not control. --> Privacy
- you can pin a specific model and version, so it will not silently change under you the way a hosted model can. The same weights give the same behaviour next month. --> Reproducibility
- you can use them offline on a laptop, on a plane, or on an air-gapped system.

However, there are trade-offs:

- You might not have the hardware to run these models. Larger models need a lot of RAM and ideally a GPU — roughly, a model needs memory on the order of its size on disk. 
- A model small enough to run on a laptop (often 7–14 billion parameters) is generally weaker than the frontier hosted models.
- It requires more technical knowledge to set up these local models, and you are the one responsible for maintaining them. 

::: {.callout-note}
A reasonable middle path: use a **local** model for anything touching sensitive data (drafting, summarising column descriptions, quick code questions), and reserve **hosted** frontier models for tasks that need the extra capability and contain nothing you would not post publicly.
:::

::: {.callout-note}
The local models still hallucinate and need the same verification than the commercial models. 
:::
...
Even though ChatGPT has only been widely used for a few years years, it has already started leaving its traces in how we speak. Words like `delve` and `meticulous` are being used more frequently in academic YouTube talks.
...
What are some markers you see in text that make you suspicious that it might have been generated by a LLM?
:::
...
On the other hand, AI can help you in being more reproducible, and your reproducibility practices can be a boon for AI models and agents. A 2026 essay by Lorena Barba, [*Reproducibility in the Age of Agentic AI*](https://arxiv.org/abs/2609.11728), argues that the reproducible-research habits we have encountered and practiced in this course — version control, tests, a clear repository layout, and written records of your decisions — can be considered "context" of the project. This information can help an AI assistant or a human collaborator to work on a project without you sitting next to them.

She argues that:

- The cost of keeping these habits has dropped, because an assistant can draft a test, a commit message, or a README section in seconds — the friction that used to make people skip reproducible habits is largely gone.

- Reproducibility now pay off immediately, not only years later: the information that protects a future student from reinventing the wheel is what an assistant uses to check its own work today.

At the same time, Barba emphasizes that you as the author remain the owner and verifier of everything an assistant drafts in your name: an agent can write the test, but only you can decide whether it tests the right thing.

## A framework for working with AI

The [AI Fluency framework](https://aifluencyframework.org/), developed by Rick Dakan and Joseph Feller in collaboration with Anthropic, distinguishes three ways of working with a model:

- **Automation** — you define a task and hand it off to a model.
- **Augmentation** — you and the model work together, back and forth, on a problem.
- **Agency** — you configure a model to pursue a goal on its own.

![Automation, augmentation, and agency (Dakan and Feller, with Anthropic)](figures/ai/ai-fluency-automation-augmentation-agency.jpg)

If you want to learn more, you can have a look at their free [course](https://academy.claude.com/courses/ai-fluency-framework-foundations).

However, no matter how you work with AI, working well and effectively can be desribed by the **four D's**:

![The four D's of AI fluency (Dakan and Feller, with Anthropic)](figures/ai/ai-fluency-4d-framework.jpg)

- **Delegation** — deciding whether, when, and how to involve AI at all. Successful delegation requires both domain expertise and thorough understanding of AI. The goal of using AI is not to pass every single task to AI, but to create an effective human-AI partnership.
- **Description** — describing what you want clearly enough to get a useful result. Description is about communicating with AI in a way that ensures that it can produce the product you are aiming to produce. Prompt engineering is an important aspect.
- **Discernment** — judging the quality of what comes back rather than accepting it. Discernment relies on the user’s ability to evaluate the products produced by AI, the process followed to produce the product, and how it behaves. Human oversight in the generation process is important for every single LLM. 
- **Diligence** — taking responsibility for what you do with the output and how you got there, including being honest about when and how you used AI. Thoughtful diligence is important to ensure that the use of AI collaborations is effective, efficient, ethical, and safe
